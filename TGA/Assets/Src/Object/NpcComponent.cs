        ...
        if (collision.gameObject.tag == "Player")
        {
            if (!useTrigger)
            {
                if (Input.GetKey(KeyCode.Z))
                {
                    ...
                    attribute[na].icon.SetActive(true);

                    PlayerParameter pp = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerParameter>();
                    foreach (KeyValuePair<Parameter, int> pair in attribute[na].playerProperty)
                    {
                        int parameter = pp.GetParameter(pair.Key) + pair.Value;
                        pp.SetParameter(pair.Key, parameter);
                    }
